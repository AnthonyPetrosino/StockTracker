...
def live_update(request):
    ...
    for stock in user_stocks:
        ...
        if data.get("quoteType") in ["MUTUALFUND", "ETF"]:
            ...
            )  # For ETFs/mutual funds, use previous close
        else:
            current_price = Decimal(
                data.get(
                    "currentPrice", data.get("ask", data.get("regularMarketPrice", 0))
                )
            )  # For stocks, use current price
    ...
    profile.total_value = (
        available_cash + total_stock_value
...
def fetch_etf_holdings(etf_symbol):
    ...
    try:
        ...
        try:
            ...
            holdings_section = driver.find_element(
                By.XPATH, '//*[@data-testid="top-holdings"]'
            )
            percents = holdings_section.find_elements(
                By.CSS_SELECTOR, "span.data.yf-1ix710n"
            )
            ...
            for holding, percent in zip(holdings, percents):
                ...
                holding_percent = (
                    float(percent.text.strip().replace("%", "")) / 100
                )  # Convert to decimal
                holding_symbol = holding.get_attribute("href").split("/")[
                    -1
...
def assetCalc(request):
    if request.method == "POST":
        etf_symbols = request.POST.getlist(
            "etf"
        )  # Get the ETF symbols from the form (allowing multiple symbols)
        stocks_owned = request.POST.getlist(
            "stocksOwned"
        )  # Get corresponding stock quantities
        ...
        for etf_symbol, stocks_owned_qty in zip(etf_symbols, stocks_owned):
            ...
            if (
                "symbol" not in etf_info
                or "delisted" in etf_info.get("status", "").lower()
            ):
                messages.error(
                    request,
                    f"Invalid or delisted ETF symbol: {etf_symbol}. Please enter a valid ETF symbol.",
                )
                return redirect(
                    "assetCalc"
                )  # Redirect to the assetCalc view or your desired page
            ...
            for holding in etf_holdings:
                ...
                holding_value_in_etf = (
                    (holding_percentage / 100) * etf_price * stocks_owned_qty
                )  # Calculate value
                ...
                if stock_symbol not in combined_holdings:
                    ...
                    combined_holdings[stock_symbol]["holding_value"] += (
                        holding_value_in_etf
                    )
...
def execute_trade(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            trade_type = form.cleaned_data.get(
                "trade_type"
            )  # Safely access cleaned_data
            company_name = query
            ...
            if not data:
                messages.error(
                    request,
                    "Failed to retrieve stock data. Please check the stock symbol.",
                )
                return redirect("buySell")
            ...
            if data.get("quoteType") in ["MUTUALFUND", "ETF"]:
                ...
                stock_price = Decimal(
                    data.get(
                        "currentPrice",
                        data.get("ask", data.get("regularMarketPrice", 0)),
                    )
                )

            if stock_price <= 0:
                messages.error(
                    request, "Invalid stock symbol or stock has been delisted."
                )
                return redirect("buySell")
            ...
            if is_buy:
                if available_cash >= total_value:
                    ...
                    user_stock, created = UserStock.objects.get_or_create(
                        ...
                        defaults={
                            "stock_quantity": 0,
                            "stock_price": stock_price,
                            "stock_purchase_date": timezone.now(),
                        },
                    )
                    ...
                    if order_type == "shares":
                        messages.success(
                            request,
                            f"Successfully bought {stock_quantity:.2f} shares of {company_name}.",
                        )
                    else:
                        messages.success(
                            request,
                            f"Successfully bought {stock_quantity / stock_price:.2f} shares of {company_name}.",
                        )
                else:
                ...
                if user_stock and user_stock.stock_quantity >= stock_quantity:
                    ...
                    total_value = (
                        stock_quantity * stock_price
                    )  # Assuming quantity is the number of shares sold
                    profile.available_cash += total_value
                    profile.save()
                    ...
                    messages.success(
                        request, f"Sold {stock_quantity:.2f} shares of {company_name}."
                    )
                else:
            ...
            messages.error(
                request,
                "There was an error in your form submission or you entered a value less than 0.02. Please check your inputs.",
            )
...
def stock_forecast_view(request):
    ...
    if "symbol" not in stock_info or "delisted" in stock_info.get("status", "").lower():
        messages.error(
            request,
            f"Invalid or delisted stock symbol: {selected_stock}. Please enter a valid stock symbol.",
        )
    ...
    try:
        ...
        formatted_stock_info = {
            ...
            "trailingAnnualDividendRate": stock_info.get(
                "trailingAnnualDividendRate", "N/A"
            ),
            "trailingAnnualDividendYield": stock_info.get(
        ...
        if timestamp != "N/A":
            try:
                ...
                formatted_stock_info["fundInceptionDate"] = (
                    formatted_date  # Update the formatted date in the dictionary
                )
            except (ValueError, TypeError):
                formatted_stock_info["fundInceptionDate"] = (
                    "Invalid Date"  # Handle cases where conversion fails
                )
        else:
            formatted_stock_info["fundInceptionDate"] = (
                "N/A"  # If no valid timestamp, keep it as N/A
            )

        # Turns it into comma string (e.g, x,xxx,xxx,xxx)
        formatted_stock_info["marketCap"] = (
            "{:,}".format(formatted_stock_info["marketCap"])
            if isinstance(formatted_stock_info["marketCap"], int)
            else formatted_stock_info["marketCap"]
        )
        formatted_stock_info["averageVolume"] = (
            "{:,}".format(formatted_stock_info["averageVolume"])
            if isinstance(formatted_stock_info["averageVolume"], int)
            else formatted_stock_info["averageVolume"]
        )
        formatted_stock_info["floatShares"] = (
            "{:,}".format(formatted_stock_info["floatShares"])
            if isinstance(formatted_stock_info["floatShares"], int)
            else formatted_stock_info["floatShares"]
        )
        formatted_stock_info["volume"] = (
            "{:,}".format(formatted_stock_info["volume"])
            if isinstance(formatted_stock_info["volume"], int)
            else formatted_stock_info["volume"]
        )
        formatted_stock_info["totalAssets"] = (
            "{:,}".format(formatted_stock_info["totalAssets"])
            if isinstance(formatted_stock_info["totalAssets"], int)
            else formatted_stock_info["totalAssets"]
        )
        formatted_stock_info["averageDailyVolume10Day"] = (
            "{:,}".format(formatted_stock_info["averageDailyVolume10Day"])
            if isinstance(formatted_stock_info["averageDailyVolume10Day"], int)
            else formatted_stock_info["averageDailyVolume10Day"]
        )

        # Turns it into percents %
        formatted_stock_info["dividendYield"] = (
            f"{(formatted_stock_info['dividendYield'] * 100):.2f}%"
            if isinstance(formatted_stock_info["dividendYield"], (int, float))
            else formatted_stock_info["dividendYield"]
        )
        formatted_stock_info["earningsGrowth"] = (
            f"{(formatted_stock_info['earningsGrowth'] * 100):.2f}%"
            if isinstance(formatted_stock_info["earningsGrowth"], (int, float))
            else formatted_stock_info["earningsGrowth"]
        )
        formatted_stock_info["trailingAnnualDividendYield"] = (
            f"{(formatted_stock_info['trailingAnnualDividendYield'] * 100):.2f}%"
            if isinstance(
                formatted_stock_info["trailingAnnualDividendYield"], (int, float)
            )
            else formatted_stock_info["trailingAnnualDividendYield"]
        )
        formatted_stock_info["52WeekChange"] = (
            f"{(formatted_stock_info['52WeekChange'] * 100):.2f}%"
            if isinstance(formatted_stock_info["52WeekChange"], (int, float))
            else formatted_stock_info["52WeekChange"]
        )
        formatted_stock_info["fiveYearAverageReturn"] = (
            f"{(formatted_stock_info['fiveYearAverageReturn'] * 100):.2f}%"
            if isinstance(formatted_stock_info["fiveYearAverageReturn"], (int, float))
            else formatted_stock_info["fiveYearAverageReturn"]
        )
        formatted_stock_info["threeYearAverageReturn"] = (
            f"{(formatted_stock_info['threeYearAverageReturn'] * 100):.2f}%"
            if isinstance(formatted_stock_info["threeYearAverageReturn"], (int, float))
            else formatted_stock_info["threeYearAverageReturn"]
        )
