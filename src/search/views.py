...
def search(request):
    query = request.GET.get(
        "q", default="AAPL"
    )  # Get search query from the form, default to AAPL if none
    ...
    if query:
        ...
        return render(
            request, "search/search.html", {"stock_info": stock_info, "query": query}
        )
    else:
        return render(
            request, "search/search.html", {"error": "No stock symbol provided."}
        )
            ...
            extra_charts_data.append(
                {
                    ...
                    "Price": related_quote.info.get(
                        "currentPrice", related_quote.info.get("previousClose", 0)
                    ),
